<template>
  <OModal :active="active" @close="closeDialog">
    <div class="w-full rounded bg-white p-2 sm:w-full sm:p-4">
            <form @submit.prevent="deletemanager">
...
export default {
  ...
  methods: {
    deletemanager() {
      this.$axios
        .$delete(`/v1/managers/${this.data.id}`)
        .catch(() => {
...
::v-deep .v-text-manager.v-text-manager--solo:not(.v-text-manager--solo-flat)
...
::v-deep .v-text-manager input::placeholder {
